...
sha1.from_hex = function(hex)
	...
	for letter in hex:gmatch('.') do
		local b = sha1.hex_to_bits[letter]
		assert(b)
...
sha1.ADD = function(first, ...)

	local a = sha1.COPY(first)
...
sha1.asHEX = function(a)
	...
	while i < #a do
		...
		hex = sha1.binary_to_hex[binary] .. hex
...
sha1.x67452301 = sha1.from_hex("67452301")
sha1.xEFCDAB89 = sha1.from_hex("EFCDAB89")
sha1.x98BADCFE = sha1.from_hex("98BADCFE")
sha1.x10325476 = sha1.from_hex("10325476")
sha1.xC3D2E1F0 = sha1.from_hex("C3D2E1F0")

sha1.x5A827999 = sha1.from_hex("5A827999")
sha1.x6ED9EBA1 = sha1.from_hex("6ED9EBA1")
sha1.x8F1BBCDC = sha1.from_hex("8F1BBCDC")
sha1.xCA62C1D6 = sha1.from_hex("CA62C1D6")


sha1.sha1 = function(msg)

	assert(type(msg) == 'string')
	assert(#msg < 0x7FFFFFFF) -- have no idea what would happen if it were large

	local H0 = sha1.x67452301
	local H1 = sha1.xEFCDAB89
	local H2 = sha1.x98BADCFE
	local H3 = sha1.x10325476
	local H4 = sha1.xC3D2E1F0
	...
	while chunk < chunks do
		...
		for t = 0, 15 do
			W[t] = sha1.from_hex(string.format("%02x%02x%02x%02x", msg:byte(start, start + 3)))
			start = start + 4
		end
		...
		for t = 16, 79 do
			...
			W[t] = sha1.ROTATE(1, sha1.XOR(W[t-3], W[t-8], W[t-14], W[t-16]))
		end
		...
		for t = 0, 79 do
			if t <= 19 then
				...
				f = sha1.OR(sha1.AND(B, C), sha1.AND(sha1.NOT(B), D))
				K = sha1.x5A827999
			elseif t <= 39 then
				-- B XOR C XOR D
				f = sha1.XOR(B, C, D)
			...
			elseif t <= 59 then
				...
				f = sha1.OR3(sha1.AND(B, C), sha1.AND(B, D), sha1.AND(C, D))
				K = sha1.x8F1BBCDC
			else
				-- B XOR C XOR D
				f = sha1.XOR(B, C, D)
			...
			TEMP = sha1.ADD(sha1.ROTATE(5, A), f, E, W[t], K)
...
sha1.hex_to_binary = function(hex)
	return hex:gsub('..', function(hexval)
		return string.char(tonumber(hexval, 16))
	end)
end

sha1.sha1_binary = function(msg)
	return sha1.hex_to_binary(sha1.sha1(msg))
end
...
sha1.blocksize = 64 -- 512 bits

sha1.hmac_sha1 = function(key, text)
	...
	if #key > sha1.blocksize then
		key = sha1.sha1_binary(key)
	end

	local key_xord_with_0x36 = key:gsub('.', sha1.xor_with_0x36) .. string.rep(string.char(0x36), sha1.blocksize - #key)
	local key_xord_with_0x5c = key:gsub('.', sha1.xor_with_0x5c) .. string.rep(string.char(0x5c), sha1.blocksize - #key)

	return sha1.sha1(key_xord_with_0x5c .. sha1.sha1_binary(key_xord_with_0x36 .. text))
end

sha1.hmac_sha1_binary = function(key, text)
	return sha1.hex_to_binary(sha1.hmac_sha1(key, text))
end
